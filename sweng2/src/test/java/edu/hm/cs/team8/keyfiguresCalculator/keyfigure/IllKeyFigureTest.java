...
public class IllKeyFigureTest {

	private Set<TimeTrackingEntry> entries;

	private IKeyFigure ill;
	...
	public void test() {
		...
		KeyFigureResult result = ill.calculate(KeyFigures.ILL,
				entries);
		
		Assert.assertEquals(result.getValue(), new Double(8));
